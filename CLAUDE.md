...
10. **不得為了讓數字好看而改判準。** 判準校準**月度不得週調**
    （安裝到實際呼叫的回饋延遲以月計）。
11. **★ 多語描述必須寫進 frontmatter 的 `description`，不能只放內文。**
    `description` **就是觸發器** —— 譯文只放內文，等於該語系的使用者叫不動這支 skill。
    `zh-TW` 是 canonical，譯文與正本不一致時**錯的是譯文**；
    `metadata.omsn-description-languages` 宣告了幾種，`check_catalog.py` 就驗幾種。
12. **★ 安全警語不得只存在於一種語言。** 每一語的描述都要帶三級分類、
    白名單在三平台失效、Vercel 可直接採購這三條 ——
    **把警語留在別種語言裡，等於沒有寫。**
    未翻譯的章節一律標 ⬜ 並印在涵蓋表上：**「還沒翻」與「不需要翻」意思相反。**
